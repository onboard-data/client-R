...
#'
create_building <- function(building_name,
                            org_id = NULL,
                            verbose = TRUE) {
  ...
  building_json <- data.frame(name = building_name, org_id = org_id) %>%
    toJSON()
  
  building_json <- gsub("\\[|\\]", "", building_json)
  
  create_building <- api.post(endpoint = "buildings", json_body =  building_json)
  
  if (verbose) {
    print(sprintf(
      "Success!! Building %s created for %s\n",
      building_name,
      org_name
    ))
  }
